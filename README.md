...
项目地址：https://github.com/andyRon/ARGuliCollege
...
谷粒学院，是一个**B2C模式**的职业技能在线教育系统，分为**前台用户系统**和**后台运营平台**。
...
-- 添加逻辑删除字段
ALTER TABLE user 
ADD COLUMN deleted INT DEFAULT 0 COMMENT '逻辑删除标识 0-未删除 1-已删除';
...
雪花算法的原理就是生成一个的 64 位比特位的long类型的唯一 id。
...
乐观锁是解决特定问题的一种解决方案，主要解决**==丢失更新==**。（通过版本号机制来检测并发冲突，避免了数据库的排他锁。）
...
>
> 6. 单表行数超过500万行或者单表容量超过 2GB，才推荐进行分库分表。 说明：如果预计三年后的数据量根本达不到这个级别，请不要在创建表时就分库分表。
>
